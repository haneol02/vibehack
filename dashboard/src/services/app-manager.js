  ...
  async start(projectId, projectSlug, startCommand = 'npm start', appPort = 3000) {
    ...
    if (startCommand === 'npm start') {
      try {
        ...
        if (!scripts.start) {
          if (scripts.dev) startCommand = 'npm run dev -- --port $PORT --host --allowed-hosts all';
          else if (scripts.preview) startCommand = 'npm run preview -- --port $PORT --host --allowed-hosts all';
        }
      } catch {}
    }
